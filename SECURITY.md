...
The newest released `0.2.x` version receives security fixes. This project is experimental and depends on a changing third-party web UI, so upgrade to the latest patch before reporting an adapter problem.
...
- Roster and classroom-skill writes require a fresh preview plus `confirm: true`; this is a safety gate, not an authorization system.
